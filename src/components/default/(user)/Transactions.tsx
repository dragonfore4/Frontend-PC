...
const Transactions = () => {
    ...
    const fetchData = async () => {
        ...
        if (auth && auth.user && response.ok) {
            ...
            console.log(user_id)
            setTransactions(data.filter((d: Transaction) => d.buyer_id === user_id.user_id));
        }
    };
    ...
    return (
        <div className='relative h-full'>
            ...
            {
                (
                    // <div className={`absolute w-full h-full bg-slate-500  rounded-lg shadow-xl top-0 text-white overflow-y-auto transition-all ease-in-out duration-700 ${isOpenSlip ? "opacity-100" : "opacity-0"} `}>
                    <div
